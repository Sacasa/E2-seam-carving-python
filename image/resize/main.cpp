...
cv::Mat add_vertical(int n, cv::Mat image, cv::Mat gradient){
  ...
  for(int i =0; i < n; i++){
    ...
    int* list_x_seams = seams_vertical<uchar>(gradient);
    result = insert_seams_vertical<cv::Vec3b>(result, list_x_seams);
    gradient = insert_seams_gradient_vertical<uchar>(gradient, list_x_seams);
    free(list_x_seams);
...
cv::Mat remove_vertical(int n, cv::Mat image, cv::Mat gradient){
  ...
  for(int i =0; i < n; i++){
    ...
    gradient = move_im_vertical<uchar>(gradient, list_x_seams);
    free(list_x_seams);       
...
int main( int argc, char** argv ) {
  ...
  cv::Mat image, image_gray, gradient, result, result_x;
  ...
  int x_to_remove = image_x - atoi(argv[2]);
  int y_to_remove =  image_y - atoi(argv[3]);
  std::cout << "x_to_remove" << x_to_remove << "y_to_remove" << y_to_remove << std::endl;

  if(x_to_remove < 0)
    result_x = add_vertical(-x_to_remove,image,gradient);
  else
    result_x = remove_vertical(x_to_remove,image,gradient);
